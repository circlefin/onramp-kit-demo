...
- [Security and Usage Model](#security-and-usage-model)
...
[Circle Developer Console](https://console.circle.com/api-keys)
...
2. Create your local environment file:
  ...
  ```
3. Add your Circle kit key to `.env.local`:
  ```bash
  ...
  ```
   ...
   control.
   Leave the sandbox endpoints copied from `.env.example` in place:
   Use a sandbox kit key with these endpoints. Removing either line points the
   app at mainnet, where purchases involve real funds.
...
5. Open [http://localhost:3000](http://localhost:3000), open the **Wallet**
  tab to connect MetaMask, then select **Add money** on **Home**.
   ...
   you switch to popup mode with `NEXT_PUBLIC_EMBEDDED_ONRAMP=0`, your browser
   must allow popups for the application.
...
Setting `NEXT_PUBLIC_EMBEDDED_ONRAMP` to `0` or `false` changes step 4: the
hosted experience opens in a popup instead of the embedded sheet.
...
| `NEXT_PUBLIC_EMBEDDED_ONRAMP`        | Browser     | Embedding the hosted experience in the phone UI is the default. Set to `0` or `false` to open it in a popup instead.          |
...
`NEXT_PUBLIC_EMBEDDED_ONRAMP`.

> [!CAUTION]
> There is no in-app confirmation before you leave sandbox. Deleting,
> commenting out, or blanking `ONRAMP_API_BASE_URL` or
> `NEXT_PUBLIC_ONRAMP_WIDGET_BASE_URL` silently switches that half of the
> integration to production/mainnet, where transactions settle with real money
> and cannot be reversed. Change them only for a deployment you intend to run
> against production, and set both together so the server and the widget stay
